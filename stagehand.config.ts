...
const StagehandConfig: ConstructorParams = {
  ...
  localBrowserLaunchOptions:{
    ...
    recordVideo: { dir: '/path/to/videos', size: { width, height } },
    // executablePath: '/Applications/Google Chrome.app', // Custom path to the Chrome executable.
  }
...
export function logLineToString(logLine: LogLine): string {
  ...
  try {
    if (logLine.auxiliary?.error) {
      return `[stagehand:${logLine.category}`.padEnd(30)+`-> ${logLine.message}\n ${logLine.auxiliary.error.value}\n ${logLine.auxiliary.trace.value}`;
    }
